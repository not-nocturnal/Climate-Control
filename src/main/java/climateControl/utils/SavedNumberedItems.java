...
abstract public class SavedNumberedItems<Type extends SelfTrackable<Type>> {
    ...
    public int nextInt(World world) {
        return world.mapStorage.getUniqueDataId(saveTitle);
    }
    ...
    public Type saved(int index, World world) {
        try {
            return saver.savee(saveName(index), world, maker(index), saveOnNew(index))
                .data();
        } catch (Exception e) {
            ...
            // ConfigManager.logger.info(saver.savee(saveName(index), world, maker(index),
            // saveOnNew(index)).toString());
            throw new RuntimeException();
        }
    }

    public Type saved(int index, World world, Maker<Type> maker) {
        return saver.savee(saveName(index), world, maker, saveOnNew(index))
            .data();
    }
}

...
public abstract class Savee<Type extends SelfTrackable<Type>> extends WorldSavedData {
    ...
    private static Accessor<MapStorage, Map> loadedMapAccess = new Accessor<MapStorage, Map>(
        "field_75749_b",
        "loadedDataMap");

    private static Accessor<MapStorage, List> loadedListAccess = new Accessor<MapStorage, List>(
        "field_75750_c",
        "loadedDataList");
    ...
    public void loadData(String saveName, Savee<Type> worldsaveddata, World world, Maker<Type> maker) {
        ...
        final MapStorage mapStorage = world.mapStorage;;
        if (saveHandler != null) {
            try {
    ...
    public Savee(String saveName, Type data, boolean startDirty) {
        super(saveName);
        this.saveName = saveName;
        if (startDirty) set(data); // also dirties
        else {
            this.data = data;
        }
        data.informOnChange(toSelf);
    ...
    public void readFromNBT(NBTTagCompound nbttagcompound) {
        DataInput inputStream = new DataInputStream(new ByteArrayInputStream(nbttagcompound.getByteArray(DATA_NAME)));
        try {

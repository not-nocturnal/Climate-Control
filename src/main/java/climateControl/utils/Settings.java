...
abstract public class Settings implements Streamable, ConfigReader {
    ...
    public void copyTo(Configuration target) {
        ...
        for (Category category : categories.values()) {
            if (category.description()
                .length() > 0) target.addCustomCategoryComment(category.name, category.description());
        }
    }
    ...
    protected class BooleanSetting extends Setting<Boolean> {
        ...
        public Property propertyFrom(Configuration source) {
            if (comment.equals("")) return source.get(category.name, key, value());
            return source.get(category.name, key, value(), comment);
        }

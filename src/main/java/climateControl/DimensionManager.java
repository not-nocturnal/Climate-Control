...
public class DimensionManager {
    ...
    public DimensionManager(ClimateControlSettings newSettings, CCDimensionSettings dimensionSettings,
    ...
    private GenLayerRiverMix patchedGenLayer(ClimateControlSettings settings, WorldType worldType, long worldSeed) {
        ...
        if (settings.vanillaLandAndClimate.value()) {
            ...
            newMix = new CorrectedContinentsGenerator(settings, this.configDirectory.getParentFile())
                .fromSeed(worldSeed, worldType);
        }
        ...
        if (newMix instanceof GenLayerLowlandRiverMix) {
            ((GenLayerLowlandRiverMix) newMix).setMaxChasm(
    ...
    private ClimateControlSettings defaultSettings(MinecraftFilesAccess dimension, boolean newWorld) {
        ...
        if (!dimension.baseDirectory()
            .exists()) {
            dimension.baseDirectory()
                .mkdir();
            if (!dimension.baseDirectory()
                .exists()) {}
        }
    ...
    private boolean ignore(WorldType considered, ClimateControlSettings settings) {
        ...
        if (settings.interveneInBOPWorlds.value()) {
            if (considered.getWorldTypeName()
                .equalsIgnoreCase("BIOMESOP")) return false;
        }
        if (settings.interveneInHighlandsWorlds.value()) {

            if (considered.getWorldTypeName()
                .equalsIgnoreCase("Highlands")) return false;
            if (considered.getWorldTypeName()
                .equalsIgnoreCase("HighlandsLB")) return false;
        }
        if (true) {
            if (considered.getWorldTypeName()
                .equalsIgnoreCase("FWG")) return false;
        }

        if (considered.getWorldTypeName()
            .equalsIgnoreCase("RTG")) return false;
        return true;
    }
    ...
    public void onCreateSpawn(WorldEvent.CreateSpawnPosition event) {
        ...
        if (event.settings.isBonusChestEnabled()) {
            ...
            for (int i = 0; i < 100; ++i) {
                int j = world.getWorldInfo()
                    .getSpawnX() + rand.nextInt(6 + i / 10)
                    - rand.nextInt(6 + i / 10);
                int k = world.getWorldInfo()
                    .getSpawnZ() + rand.nextInt(6 + i / 10)
                    - rand.nextInt(6 + i / 10);

                int l = world.getTopSolidOrLiquidBlock(j, k) + 1;

                if (world.getBlock(j, l, k)
                    .equals(Blocks.air)) {
    ...
    public void onWorldLoad(World world) {
        ...
        if (world instanceof WorldServer && worldSeed != 0) {
            ...
            if (world.getWorldInfo()
                .getWorldTotalTime() < 100) {
                // new world
                newWorld = true;
            }
            currentSettings = dimensionalSettings(dimensionAccess, newWorld);
            // logger.info(""+dimension +" "+ currentSettings.snowyIncidence.value() +" "+
            // currentSettings.coolIncidence.value());
            ...
            try {
                ...
                if (patched != null) {
                    ...
                    this.riverLayerWrapper(dimension)
                        .lock(dimension, world, currentSettings);
                } else {
                    // lock manually
                ...
                if (currentSettings.vanillaLandAndClimate.value() == false) {
                    if (currentSettings.noGenerationChanges.value() == false) {
                        ...
                        if (world.getWorldInfo()
                            .getWorldTotalTime() < 40000) {
                            ArrayList<PlaneLocation> existingChunks = new ChunkLister()
                                .savedChunks(levelSavePath((WorldServer) world));
             ...
             * //this.biomeLocker.lock(riverMixBiome.get(activeRiverMix), dimension,
             * this.servedWorlds.get(dimension),newSettings);
             * } else {
    ...
    private void salvageSpawn(World world) {
        ...
        while (spawnY < 64) {
            ...
            logger.info(
                "checking for spawn at " + (x + xMove * 16)
                    + ","
                    + (z + zMove * 16)
                    + "move "
                    + move
    ...
    private String levelSavePath(WorldServer world) {
        String result = "";
        result = world.getChunkSaveLocation()
            .getAbsolutePath();
        return result;
    }

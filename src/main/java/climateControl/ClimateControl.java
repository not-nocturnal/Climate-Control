...
@Mod(modid = "climatecontrol", name = "Climate Control", version = Tags.VERSION, acceptableRemoteVersions = "*")
...
public class ClimateControl {
    ...
    public void postInit(FMLPostInitializationEvent event) throws Exception {
        ...
        config.save();
        logger.info(
            "biome setting count: " + BiomePackageRegistry.instance.biomeSettings()
                .size());
    ...
    public void serverStarted(FMLServerStartedEvent event) {
        ...
        String worldName = MinecraftServer.getServer()
            .getFolderName();
        if (MinecraftServer.getServer()
            .isSinglePlayer()) {
            File saveDirectory = MinecraftServer.getServer()
                .getFile("saves");
            worldSaveDirectory = new File(saveDirectory, worldName);
        } else {
            PropertyManager settings = new PropertyManager(
                MinecraftServer.getServer()
                    .getFile("server.properties"));
            worldName = settings.getProperty("level-name", worldName);
            worldSaveDirectory = MinecraftServer.getServer()
                .getFile(worldName);
        }
    ...
    public void onServerStarting(FMLServerStartingEvent event) {
        ...
        File directory = event.getServer()
            ...
            logger.info(
                biome.biomeName + " "
                    ...
                    + biome.getTempCategory()
                        .toString()
                    + " "
                    + biome.getFloatTemperature(0, 64, 0)
                    + " rain "
                    + biome.getFloatRainfall());
        }
    }
